...
<!-- MODAL DELETE END -->
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="card">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 table-responsive">
                    <div class="card-body">
                        <table class="table table-hover dts" id="dts">
                            <thead>
                                <tr>
                                    <th scope="col">#</th>
                                    <th scope="col" class="text-center">Abreviatura</th>
                                    ...
                                    @foreach ($programas as $info)
                                        ...
                                        <td>
                                            ...
                                            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                                data-bs-target="#modalshow{{ $info->id_programa }}"><i
                                                    class="fa-solid fa-eye"></i></button>

                                            <!-- Button edit modal -->
                                            <button type="button" class="btn btn-warning" data-bs-toggle="modal"
                                                data-bs-target="#exampleModal{{ $info->id_programa }}"><i
                                                    class="fa-solid fa-pen-to-square"></i></button>

                                            <!-- Button delete modal -->
                                            @if ($info->activo > 0)
                                                <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                                    data-bs-target="#deleteModal{{ $info->id_programa }}"><i
    ...
    @foreach ($programas as $programa)
        <div class="modal fade" id="deleteModal{{ $programa->id_programa }}" tabindex="-1" aria-labelledby="deleteModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="deleteModalLabel">Eliminar registro | {{ $programa->abreviatura }}
                        </h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body text-center">
                        <form action="{{ route('areasmetas.destroy', $programa) }}" method="post">
                            @csrf @method('DELETE')



                            <p><strong>{{ $programa->id_areasmetas }}</strong></p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary">Cancelar</button>

                        <a href="{{ route('programas.destroy', ['programa' => $programa->id_programa]) }}">
                            <button type="submit" class="btn btn-danger" data-bs-dismiss="modal">BORRAR</button>
                        </a>
                    </div>
        ...
        <div class="modal fade" id="exampleModal{{ $info->id_programa }}" tabindex="-1"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('editProgram', ['id' => $info->id_programa]) }}" method="POST"
